...
For each dangerous pattern found, map it to an Enact construct. Use the `@action`
decorator to wrap the user's existing functions — no connector classes needed:

| Pattern found | Enact suggestion |
|---------------|-----------------|
| `requests.delete/post/put` | `@action("http.delete_resource")` + custom URL allowlist policy |
| `os.remove` / `shutil.rmtree` | `@action("fs.delete_file")` + `dont_delete_file` policy |
| `subprocess.run` | `@action("shell.run_command")` + custom command allowlist policy |
| DB `INSERT`/`UPDATE`/`DELETE` | `@action("db.insert_row")` etc + `dont_delete_without_where` |
| GitHub repo mutations | `@action("github.create_pr")` etc + `dont_push_to_main` |
| Slack messages | `@action("slack.post_message")` + `require_channel_allowlist` |

For rollback support, pair actions with `fn.rollback_with(inverse_fn)`.
...
# Wrap your existing functions with @action — no connector classes needed
@action("github.create_pr")
def create_pr(repo, branch, title):
    # your existing code stays exactly the same
    return github_sdk.create_pr(repo=repo, branch=branch, title=title)
...
# Optional: pair forward/backward actions for rollback
...
enact = EnactClient(
    actions=[create_pr, close_pr, write_output],
    policies=[
        dont_push_to_main,
        dont_delete_file,
...
Then ask:
> "This is what I'd write to `enact_setup.py`. I'd also update your agent files
> to call `enact.run_action()` instead of calling APIs directly.
...
5. For each agent file containing dangerous calls:
   ...
   - Replace direct API calls with `enact.run_action("system.action_name", ...)`
